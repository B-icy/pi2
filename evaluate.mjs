...
const prompts = {
  cli: `Build a complete dependency-free Python task tracker CLI in tasks.py. Commands: --file PATH add TITLE; --file PATH list; --file PATH done ID. The global --file option is required, and TITLE is one quoted argv string. Print JSON only on successful commands: add returns {"id":1,"title":"...","done":false}; list returns an array of those objects; done returns the updated object. IDs are increasing positive integers, titles must be nonblank, done is idempotent. Invalid IDs and corrupt files produce a clear stderr error and nonzero exit without modifying data. Missing file means an empty task list; create parent directories on add. Persist atomically and retain Unicode exactly. Importing tasks.py must have no side effects. Provide README.md and regression tests. Implement and run your tests; do not merely describe the solution.`,
